export class Log {
    ...
    returning(value?: string, ...optionalParams: any[]): void {
        if (value != undefined && value.length > 0) {
            this.dev(logColor(LogColor.bright) + `Returning "${value}"...`, ...optionalParams)
        }
        else {
            this.dev(logColor(LogColor.bright) + `Returning...`, ...optionalParams)
        }
    }
